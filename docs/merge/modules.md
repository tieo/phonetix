...
| `engines/espeak-wasm` | espeak-ng WASM and its 23 MB data; implements the `Phonemizer` port | browser (background or offscreen) | platform | none |
| `engines/bergamot-wasm` | Bergamot engine (about 5 MB) and per-direction models; implements the `Translator` port | browser (background or offscreen) | platform | `packs` for model download |
| `engines/mlkit` | ML Kit on-device translation; implements the `Translator` port; carries the attribution string | Android | platform | none |
| `engines/audio` | Web Audio playback of espeak output in the browser; Android TextToSpeech or pack audio on Android | both | platform (two implementations of one port) | `Phonemizer` |
