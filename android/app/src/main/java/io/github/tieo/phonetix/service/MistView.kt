 ...
 *
 * Carried over from Taplex, where the hover was built and where it belongs: the merged
 * product keeps that gesture rather than the ring-under-the-finger that replaced it, because
 * a circle under the hand covers the word it is being asked about.
 *
...
class MistView(context: Context) : View(context) {
    ...
    private fun sampleIconColours(context: Context): IntArray {
        val drawable = ContextCompat.getDrawable(context, io.github.tieo.phonetix.R.mipmap.ic_launcher_foreground)
