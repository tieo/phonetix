 ...
 *
 * Carried over from Taplex, where reading a whole screen rather than a word at a time was the
 * other half of the product, and which the merge dropped.
 *
...
class PageOverlayView(context: Context) : View(context) {
    ...
    fun show(found: List<Line>, settled: Boolean = true) {
        ...
        if (io.github.tieo.phonetix.BuildConfig.DEBUG) {
            // What the reader is actually reading, which is the only thing worth checking
            // about a replaced page: that the lines on screen are the page's own lines, in
            // the reader's language, where the page had them.
            val sb = StringBuilder("PAGEDREW ${found.size} ")
            for (line in found) {
                sb.append(line.bounds.left).append(',').append(line.bounds.top)
                    .append('=').append(line.text.replace('|', '/')).append('|')
            }
