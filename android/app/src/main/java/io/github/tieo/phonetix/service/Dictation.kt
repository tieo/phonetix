...
class Dictation(private val context: Context) {
    ...
    private fun listen(language: String) {
        if (!hasPermission()) {
            android.util.Log.d("Phonetix", "dictation: no permission to listen")
            return
        }
        val client = build() ?: run {
            android.util.Log.d("Phonetix", "dictation: no recogniser on this phone")
            return
        }
        ...
        client.setRecognitionListener(object : RecognitionListener {
            ...
            override fun onError(error: Int) {
                listening = false
                onState(false)
                android.util.Log.d("Phonetix", "dictation: stopped, reason $error")
                ...
                if (onDevice && error in UNSUPPORTED) {
                    onDevice = false
                    close()
                    android.util.Log.d("Phonetix", "dictation: asking the other recogniser instead")
                    listen(language)
                }
            }
        ...
        runCatching { client.startListening(intent) }
            .onFailure { android.util.Log.w("Phonetix", "listening", it) }
    }
